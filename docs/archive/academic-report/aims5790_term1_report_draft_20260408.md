# AIMS5790 Term 1 Report Draft

## Cover Sheet

Use the official cover-sheet template in `docs/write_report/AIMS5790_Cover Sheet for Final Report_Template (.docx`.
...
This is an individual project. All major tasks in the current term were completed by the author. To make the workload transparent, the task coverage of the first term is summarized below.

| Task | Description | Contribution |
| --- | --- | ---: |
| System analysis | Read, reorganized, and documented the upstream StreamDFP baseline together with the LLM extension structure | `100%` |
| Method and framework integration | Worked on the overall design logic connecting Phase1, Phase2, Phase3, policy gating, and baseline reinjection | `100%` |
| Experiment orchestration | Organized scripts, workflow wrappers, reproducibility paths, and local workbench usage for the reported runs | `100%` |
| Result diagnosis and interpretation | Compared HDD multi-model results and diagnosed the `mc1` failure-and-repair chain | `100%` |
| Report preparation | Prepared the first-term report structure, tables, and written materials for submission | `100%` |
...
The gap addressed by this project is not simply "LLMs have not been used for disk failure prediction." A more precise gap is that prior practical SMART-based prediction pipelines and semantic explanation mechanisms are often disconnected. Conventional pipelines can produce metrics but not meaningful structured causes, while free-form LLM outputs are difficult to compare fairly and difficult to feed back into the original predictor. This project addresses that gap by forcing semantic extraction to satisfy three constraints at the same time:

1. the input must remain tied to real SMART windows
2. the output must be normalized into bounded structured fields
3. the final value must be validated through the same downstream predictive metrics as the baseline

This combined constraint is what turns the project from a prompt experiment into a credible hybrid system study.
...
At the top level, the main directories are mapped as follows.
...
This subsection should be accompanied by a single architecture figure showing the classic StreamDFP path, the LLM `Phase1/2/3` extension, and the new-model calibration branch.
...
### 3.3 LLM-enhanced framework: Phase1, Phase2, and Phase3

The core method of this project is the `framework_v1` pipeline.

#### Phase1: structured window summarization
...
Phase1 is also where sampling quality becomes critical. The later `mc1` diagnosis showed that a wrong sequential sampling strategy could severely damage downstream extraction quality. Therefore, Phase1 is not just a formatting step; it is part of the experimental validity chain.

#### Phase2: offline LLM extraction and vectorization

`llm/llm_offline_extract.py` sends `summary_text` to a selected backend and extracts fields such as `root_cause`, `events`, `risk_hint`, `hardness`, and `confidence`. The project supports three backend families:
...
#### Phase3: gating, profile selection, and reinjection

`llm/scripts/build_cache_variant.py` and the Phase3 grid scripts evaluate which parts of the cache should actually be injected back into the predictor. The project commonly uses `compact9`, `compact14`, and `full70` feature profiles, combined with gate settings such as quality thresholds, severity thresholds, and rule-match constraints.
...
`new model -> no-LLM baseline -> pilot20k Phase1/Phase2/Phase3 -> guard check -> policy registration`
...
All claims in this report follow one rule: the LLM-enhanced system must be compared against the no-LLM baseline under the same downstream prediction chain and the same metric parser. The most important metrics currently used in the repository are:
...
Under the locked `Qwen3-4B-Instruct-2507 + vLLM + ZS` policy summary retained in the repository, the 12 HDD models are split as follows.
...
The current HDD summary in the repository compares three candidate models:

- `Qwen3-4B-Instruct-2507`
- `Qwen3.5-4B`
- `Qwen3.5-Plus`

The disk-model-level summary is shown below.
...
The resulting best-case Phase3 outcome is identical for all three compared models.
...
### 4.6 Phase2 quality comparison on repaired `mc1`

Although the three models converge to the same best Phase3 result on repaired `mc1`, they are not equal in Phase2 extraction quality.
...
Viewed from this perspective, `Qwen3-4B-Instruct-2507` is the strongest local default. `Qwen3.5-Plus` is the most practical API-side comparison baseline. `Qwen3.5-4B` can match the best final Phase3 result, but it does so with weaker extraction quality and higher engineering cost in the current setup.

This distinction is important for academic reporting. If the report presents only the final best-case Phase3 metric, the reader may conclude that the three models are interchangeable. The Phase2 statistics show that this would be misleading. Best-case predictive performance, extraction quality, and engineering cost are related, but they are not identical dimensions.
...
Based on the first-term results, the most reasonable current recommendation is:
...
This framing is important for the report as a whole. If the narrative overemphasizes raw gains alone, the report may read as a collection of isolated experimental wins. A stronger presentation is to show that the project established a disciplined evaluation loop, correctly diagnosed a major failure case, and produced a reproducible path for future calibration and expansion.
...
2. Phase3 remains computationally expensive, especially for cases that generate large numbers of train/test artifacts and Java simulation outputs.
...
1. Promote the current calibration branch into a more formal and reviewable model-registration pipeline.
2. Reduce the computational cost of Phase3 through stronger cache reuse and lighter variant selection.
3. Improve Phase2 robustness for long and difficult prompts.
4. Strengthen the evaluation methodology with repeated runs, uncertainty estimates, or bootstrap-style analysis where practical.
...
[3] T. Zhou, P. Niu, X. Wang, L. Sun, and R. Jin, “One fits all: Power general time series analysis by pretrained LM,” in *Advances in Neural Information Processing Systems (NeurIPS)*, 2023. [Online]. Available: https://openreview.net/forum?id=gMS6FVZvmF

[4] M. Jin, S. Wang, L. Ma, Z. Chu, J. Y. Zhang, X. Shi, P.-Y. Chen, Y. Liang, Y.-F. Li, S. Pan, and Q. Wen, “Time-LLM: Time series forecasting by reprogramming large language models,” in *The Twelfth International Conference on Learning Representations (ICLR)*, 2024. [Online]. Available: https://openreview.net/forum?id=Unb5CVPtae

[5] C. Chang, W.-Y. Wang, W.-C. Peng, and T.-F. Chen, “LLM4TS: Aligning pre-trained LLMs as data-efficient time-series forecasters,” *arXiv preprint* arXiv:2308.08469, 2023. [Online]. Available: https://arxiv.org/abs/2308.08469

[6] Z. Pan, Y. Jiang, S. Garg, A. Schneider, Y. Nevmyvaka, and D. Song, “S²IP-LLM: Semantic space informed prompt learning with LLM for time series forecasting,” in *Proceedings of the 41st International Conference on Machine Learning (ICML)*, ser. PMLR, vol. 235, pp. 39135-39153, 2024. [Online]. Available: https://proceedings.mlr.press/v235/pan24c.html

[7] S. P. Arango, P. Mercado, S. Kapoor, A. F. Ansari, L. Stella, H. Shen, H. H. J. Senetaire, A. C. Turkmen, O. Shchur, D. C. Maddix, M. Bohlke-Schneider, B. Wang, and S. S. Rangapuram, “ChronosX: Adapting pretrained time series models with exogenous variables,” in *Proceedings of the 28th International Conference on Artificial Intelligence and Statistics (AISTATS)*, ser. PMLR, vol. 258, pp. 2242-2250, 2025. [Online]. Available: https://proceedings.mlr.press/v258/arango25a.html
...
[9] N. Gruver, M. Finzi, S. Qiu, and A. G. Wilson, “Large language models are zero-shot time series forecasters,” in *Advances in Neural Information Processing Systems (NeurIPS)*, 2023. [Online]. Available: https://arxiv.org/abs/2310.07820

[10] T. Aksu, G. Woo, J. Liu, X. Liu, C. Liu, S. Savarese, C. Xiong, and D. Sahoo, “GIFT-Eval: A benchmark for general time series forecasting model evaluation,” *arXiv preprint* arXiv:2410.10393, 2024. [Online]. Available: https://arxiv.org/abs/2410.10393
...
[12] A. Bifet, G. Holmes, R. Kirkby, and B. Pfahringer, “MOA: Massive online analysis,” *Journal of Machine Learning Research*, vol. 11, no. 52, pp. 1601-1604, 2010. [Online]. Available: https://jmlr.csail.mit.edu/papers/v11/bifet10a.html

[13] Backblaze, “Backblaze Drive Stats: Download hard drive reliability stats, reports, and test data,” Backblaze. [Online]. Available: https://www.backblaze.com/cloud-storage/resources/hard-drive-test-data. [Accessed: Apr. 8, 2026].

[14] F. Xu, S. Han, P. P. C. Lee, Y. Liu, C. He, and J. Liu, “General feature selection for failure prediction in large-scale SSD deployment,” in *2021 51st Annual IEEE/IFIP International Conference on Dependable Systems and Networks (DSN)*, 2021. [Online]. Available: https://www.cse.cuhk.edu.hk/~pclee/www/pubs/dsn21.pdf
...
- a `Phase1 -> Phase2 -> Phase3` dataflow figure
...
3. Reduce Phase3 overhead through better artifact reuse, more compact candidate filtering, and cleaner workflow management.
4. Improve the robustness of Phase2 extraction, especially for longer prompts and harder cases.
...
By the end of the second term, the project is expected to produce:
