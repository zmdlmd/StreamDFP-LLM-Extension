...
`llm/window_to_text.py` transforms each SMART time window into a structured `summary_text` rather than a raw numerical dump. Typical blocks include `WINDOW`, `DATA_QUALITY`, `RULE_SCORE`, `RULE_TOP2`, `ALLOWED_EVENT_FEATURES`, `ANOMALY_TABLE`, `CAUSE_EVIDENCE`, and `RULE_PRED`. This design makes the input auditable to humans while remaining sufficiently constrained for machine extraction.
...
The extracted results are then normalized into cache records containing semantic fields and vectorized signals such as `z_llm_*`, `llm_q_score`, and rule-matching indicators. This stage constitutes the main bridge between the language model and the conventional predictive system.
