...
Note: Use the official AIMS5790 cover-sheet template as the first page of the final submission. The text below is the main body of the report.
...
RQ1. Can SMART time-window statistics be converted into a structured textual representation that is suitable for constrained LLM extraction?

RQ2. Can LLM outputs such as root causes, events, confidence, and risk hints be normalized into fixed-dimensional features that can be consumed by the original predictor?

RQ3. Should LLM enhancement be applied to all disk models, or should activation be decided at the disk-model level under explicit guards?

RQ4. How can such a hybrid Python-Java-LLM system remain reproducible and operable in a real research workflow?

1.4 Main Contributions

The project makes the following first-term contributions.

First, it extends StreamDFP into a closed-loop hybrid system rather than a loosely coupled LLM demonstration.

Second, it proposes disk-model-level policy selection instead of globally forcing LLM enhancement.

Third, it converts structured LLM extraction into comparable vectorized features such as compact9, compact14, and full70 profiles.

Fourth, it demonstrates, through the mc1 case, that a data-pipeline failure can easily be mistaken for a model failure.

Fifth, it improves experiment reproducibility through normalized workflows, result summaries, and a local workbench UI.
...
Table 1 should be inserted near this point to summarize the main modules of the repository and their responsibilities.

[Insert Figure 1 near here: Overall architecture of the StreamDFP-LLM extension.]
...
Phase 2: Offline LLM Extraction and Vectorization

llm_offline_extract.py sends summary_text to a selected backend and extracts fields such as root_cause, events, risk_hint, hardness, and confidence. The project supports three backend families: local Transformers backends, local vLLM backends, and OpenAI-compatible API backends.
...
Phase 3: Gating, Profile Selection, and Reinjection

build_cache_variant.py and the Phase 3 grid scripts evaluate which parts of the cache should actually be injected back into the predictor. The project commonly uses compact9, compact14, and full70 feature profiles, combined with gate settings such as quality thresholds, severity thresholds, and rule-match constraints.
...
[Insert Figure 2 near here: Dataflow of the Phase 1-Phase 2-Phase 3 pipeline.]
...
Table 2 should be inserted near this point to describe the input-output contract of each stage, including the main input, main output, typical artifact, and functional role.
...
- ACC(LLM) >= ACC(noLLM) - 1.0 percentage point
...
Table 3 should be inserted near this point to show the HDD disk-model policy snapshot under the retained Qwen3-4B-Instruct-2507 + vLLM + ZS baseline.
...
Table 4 should be inserted near this point to summarize the number of enabled HDD models and the average Delta Recall over accepted models.

[Insert Figure 3 near here: HDD cross-model comparison.]
...
Table 5 should be inserted near this point to compare the aligned no-LLM baseline against the best retained LLM-enhanced configuration on repaired mc1.

[Insert Figure 4 near here: mc1 repair and comparison.]

The key result is that the best retained Phase 3 outcome becomes identical for all three compared models:

- best profile: compact14, q = 0.00, sev = 0.0, rule = 0
- Recall = 100.0000
- ACC = 99.5489
- Delta Recall = +2.2296
- Delta ACC = +0.0532
...
Table 6 should be inserted near this point to compare non-unknown ratio, mapped-event ratio, average confidence, average risk hint, and average llm_q_score across the three candidate models.
...
First, there are still many historical scripts, which increases maintenance cost even though reproducibility is preserved.

Second, Phase 3 remains computationally expensive, especially for cases that generate large numbers of train/test artifacts and Java simulation outputs.

Third, local inference quality and stability still depend on backend-specific implementation details.

Fourth, the current policy mechanism is still mainly offline and disk-model-level, rather than dynamically adaptive online.

Fifth, full reproduction still depends on external resources such as local model checkpoints, GPU capacity, API quota, and storage space.

5.4 Future Work

The next stage of the project should focus on four main directions.

First, the current calibration branch should be promoted into a more formal and reviewable model-registration pipeline.

Second, the computational cost of Phase 3 should be reduced through stronger cache reuse and lighter variant selection.

Third, the robustness of Phase 2 should be improved for longer and more difficult prompts.

Fourth, the evaluation methodology should be strengthened with repeated runs, uncertainty estimates, or bootstrap-style analysis where practical.
